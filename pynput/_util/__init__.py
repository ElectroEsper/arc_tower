...
def backend(package):
    ...
    for module in modules:
        try:
            #ac.log("[stereo boxcam] - module = {}".format(str(module)))
            #ac.log("[stereo boxcam] - package = {}".format(str(package)))
            return importlib.import_module('._' + module, package)
        except ImportError as e:
            #ac.log("[stereo boxcam] Import error = {}".format(str(e)))
            errors.append(e)
            ...
            try:
                ...
                if not isinstance(e, self._HANDLED_EXCEPTIONS):
                    if not isinstance(e, AbstractListener.StopException):
                        ...
                        ac.log('Unhandled exception in listener callback')
                    self._queue.put(
